...
const CategoryDetailsPage: React.FC<CategoryDetailsPageProps> = ({
  ...
  useEffect(() => {
    const fetchEventProducts = async () => {
      try {
        ...
        console.error("Failed to fetch event products:", error);
        toast.error("Failed to fetch event products");
      }
    };
  ...
  const toggleWishlist = (product: EventProduct) => {
    ...
    if (productIndex > -1) {
      ...
      toast.success(`${product.title} removed from wishlist!`, { position: "bottom-center" });
    } else {
      updatedCart = [...existingCart, { ...product, quantity: 1 }];
      toast.success(`${product.title} added to wishlist!`, { position: "bottom-center" });
    }
  ...
  const handlePageChange = (newPage: number) => {
    router.push(`/events/${catValue}?page=${newPage}`);
  };
  ...
  {event && event.length > 0 ? (
    event.map((singleEvent) => (
      <React.Fragment key={singleEvent._id}>
        <img
          className="absolute inset-0 w-full h-full object-cover"
          src={singleEvent.singleImage}
          alt={singleEvent.category}
        />
            ...
            >
              <div onClick={() => openModalProductDetails(product._id)}>
                ...
                <div className="flex justify-center items-center flex-col gap-4">
                  ...
                  <p className="text-gray-600 text-3xl">Price: ${product.perDayPricing}/day</p>
                </div>
              </div>
              <button
              ...
              >
                ...
                {wishlistedProducts.has(product._id) ? "Remove from Wishlist" : "Add to Wishlist"}
              </button>
            </div>
          ))}
        </div>
